...
# Commit and load data into SQLite table
conn.commit()
data.to_sql('health_data', conn, if_exists='replace', index=False)

# Function to standardize age groupings
def categorize_age(age):
    """Standardizes age ranges into broader groups."""
    age_mapping = {
        "0-5": "Children (0-5)",
        "6-11": "Children (6-11)",
        "6-14": "Children (6-14)",
        "10-13": "Pre-Teens (10-13)",
        "14-17": "Teens (14-17)",
        "18-24": "Young Adults (18-24)",
        "25-44": "Adults (25-44)",
        "45-64": "Middle-Aged Adults (45-64)",
        "65+": "Seniors (65+)"
    }
...
# Connect to SQLite and load dropdown options
conn = sqlite3.connect(db_file)
...
# Load and clean age categories
age_df = pd.read_sql_query('SELECT DISTINCT Age FROM health_data', conn)
age_df["Standardized Age"] = age_df["Age"].apply(categorize_age)
ages = ["All"] + sorted(age_df["Standardized Age"].unique())
